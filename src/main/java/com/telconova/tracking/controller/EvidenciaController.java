...
public class EvidenciaController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Evidencia guardada correctamente"),
    ...
    public ResponseEntity<EvidenciaDto> uploadEvidencia(@PathVariable UUID avanceId,
        ...
        try {
            ...
            EvidenciaDto savedEvidencia = evidenciaService.uploadEvidencia(avanceId, file);
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Evidencias guardadas correctamente"),
            @ApiResponse(responseCode = "400", description = "Archivo inválido",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
            @ApiResponse(responseCode = "404", description = "Avance no encontrado",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
    ...
    @PreAuthorize("hasAnyRole('TECNICO', 'ADMIN')")
    public ResponseEntity<List<EvidenciaDto>> uploadMultipleEvidencias(@PathVariable UUID avanceId,
            @RequestParam("files") List<MultipartFile> files) {
        try {
            logger.info("Solicitud para subir {} evidencias al avance: {}", files.size(), avanceId);
    ...
    public ResponseEntity<List<EvidenciaDto>> getEvidenciasByAvance(@PathVariable UUID avanceId) {
    ...
    @PreAuthorize("hasAnyRole('TECNICO', 'ADMIN')")
    public ResponseEntity<Void> deleteEvidencia(@PathVariable UUID id) {
        logger.info("Eliminando evidencia con ID: {}", id);
